	...
	chart : function(objectResult, data,var_width,var_height,var_margin,res) {
			jsdom.env(
				...
				function(errors, window) {
					var margin = {top: var_margin, right: var_margin -10, bottom: var_margin, left: var_margin - 10},
					...
					var svg = d3.select(el).append("svg")
						 ...
						 .attr("viewBox", "0 0 "+(var_width + margin.left + margin.right)+" "+(var_height + margin.top + margin.bottom))
					  .append("g")
					...
					function capitalize(s)	{
					  ...
					  var link = svg.append("g").selectAll(".link")
					      .data(energy.links)
					    .enter().append("path")
					      .attr("class", function (d) { if (d.matchingType == "both") return "link"; else return "spec" ;})
					      .attr("d", path)
					  ...
					  link.append("title")
					      .text(function(d) { 
					      	var txt=""; 
					      	if (d.matchingType == "both") 
					      		txt="Bidirectionnal matching";
					      	else if (d.matchingType == "generalization") 
					      		txt= "Diversification from source to target";
					      	else txt ="Focus from source to target";
					      	txt+="\n"
					      	for (i in d.kernel) 
					      		txt+=d.kernel[i]+"\n"; 
					  ...
					  node.append("rect")
					      ...
					      .attr("cluster", function (d) { 
					      		if (d.name.indexOf("Classe") == -1 ) {
									return d.name.split(" ")[0].substring(1);
								}
								else {
					  ...
					  node.append("text")
					      ...
					      .attr("transform", null)
					      .text(function(d) {  
					      	var text=d.name + "-" + d.period+"\n";
					      	if (objectResult.nbLabel == 1) {
