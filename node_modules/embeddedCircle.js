...
module.exports={
	chart : function(objectResult, var_data,var_width,res) {
			jsdom.env(
				dataviz,
				function(errors, window) {
					...
					try {
						...
						if (var_data[i].children.length <= 0) {
						...
						else if (var_data[i].children.length == 1) {
							...
							node.append("circle")
							  .attr("r", function(d) { return d.children ? d.r :  d.r / 2 ; })
							  .style("opacity", "0.3");

							node.append("text")
							  .style("text-anchor", "middle")
							  .attr("y", function(d) {return d.children ? "0":"30"})
					...
					controller.embeddedCircle(res, objectResult);
					}
					catch(e) {
						res.redirect('/');
					}
