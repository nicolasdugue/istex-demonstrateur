    ...
    chart: function(objectResult,clustersSrc,clustersTarget, clustersAppeared, clustersVanished, width, height,margin,padding, res){
        jsdom.env(
            	...
            	function(errors, window) {
                    ...
                    var sizeClsSrc= ((width - 2 * margin) - (padding*(SrcList.length + clustersVanished.length))) / (SrcList.length + clustersVanished.length) + 15;
                    //logger.debug(sizeClsSrc);
                    var sizeClsTarget= ((width - 2 * margin) - (padding*(TargetList.length + clustersAppeared.length))) / (TargetList.length + clustersAppeared.length) + 15;
                    ...
                    .attr("title",function(d){return d.name.replace(/\s/g, '').split("-")[0];})
                    ...
                    .attr("class", "Cluster")
                    .attr("period", objectResult.psrc)
                    .attr("y",relativeHeightSrc - 90)
                    .attr("x",function(d,i){return (i*sizeClsSrc)+2*margin+5;});
                    if (objectResult.labels == 1) {
                        labels.data(SrcList).enter().append("text")
                        .text(function(d){return d.name.split(" - ")[1].split("/")[0];})
                        .style("font-size", "18px")
                        .style("writing-mode",  "tb")
                        .attr("period", objectResult.psrc)
                        .attr("y",relativeHeightSrc - 90)
                        .attr("x",function(d,i){return (i*sizeClsSrc)+2*margin-20;});
                    }
                    ...
                    if (objectResult.labels == 1) {
                        ...
                        .attr("period", objectResult.psrc)
                        .attr("y",relativeHeightSrc - 90)
                        .attr("x",function(d,i){return ((i+SrcList.length)*sizeClsSrc)+2*margin-20;});
                    }
                    ...
                    .style("writing-mode",  "tb")
                    .attr("period", objectResult.ptgt)
                    .attr("y",relativeHeightTarget + 20)
                    ...
                    if (objectResult.labels == 1) {
                        ...
                        .attr("period", objectResult.ptgt)
                        .attr("y",relativeHeightTarget + 20)
                        .attr("x",function(d,i){return (i*sizeClsTarget)+2*margin-20;});
                    }
                    ...
                    if (objectResult.labels == 1) {
                        ...
                        .attr("period", objectResult.ptgt)
                        .attr("y",relativeHeightTarget + 20)
                        .attr("x",function(d,i){return ((i + TargetList.length)*sizeClsTarget)+2*margin-20;});
                    }
                    ...
                    .style("stroke-width", function (d) { return (d.activity + d.activity) * 12;})
                    ...
                    .attr("src", function(d){return d.srcName.replace(/\s/g, '').split("-")[0];})
                    .attr("target", function(d){return d.targetName.replace(/\s/g, '').split("-")[0];})
                    .attr("matchingType", function(d){return d.matchingType;})
                    .attr("kernel", function(d){ return JSON.stringify(d.kernel);});
                    ...
                    .attr("class", "activity")
                    .style("font-size", "18px")
                    .attr("id", function(d){return d.srcName.replace(/\s/g, '').split("-")[0]+d.targetName.replace(/\s/g, '').split("-")[0];})
